function [result,data,RemComp,time] = LUDpipline(data)
%% Code Written Onur Ozyesil
%% [1] O. Ozyesil, A. Singer, R. Basri,
%%     Stable Camera Motion Estimation Using Convex Programming,
%%     CVPR 2015

% RemComp: record the location of cameras remained after applying LUD
% time: record the excution times for each part of LUD pipeline. The
% following code is revised because 'graphconncomp' is removed after Matlab
% 2022b, it is replaced by 'conncomp'. The change is as follows 
% (removed) [s,c]=graphconncomp(sparse([I(1,:),I(2,:)],[I(2,:),I(1,:)],ones(1,2*size(I,2)),N,N));
% (replaced) Graph = graph(sparse([I(1,:),I(2,:)],[I(2,:),I(1,:)],ones(1,2*size(I,2)),N,N));
% (replaced) [c,s] = conncomp(Graph);
% revised by Feng Yu, 2023
