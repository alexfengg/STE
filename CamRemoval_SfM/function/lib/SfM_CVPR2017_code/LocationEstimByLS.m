...
function [t_est, out] = LocationEstimByLS(AdjMat, tijMat, opts)

t_start = tic;
%% General algorithmic parameters:
%%
if ~isfield(opts, 'tolIRLS');       opts.tolIRLS = 1e-8;   end    % IRLS Tolerance
if ~isfield(opts, 'tolQuad');       opts.tolQuad = 1e-8;   end    % Inner loop tolerance
if ~isfield(opts, 'delt');          opts.delt = 1e-8;     end    % IRLS regularization parameter
...
if ~isfield(opts, 'p');       opts.p = 1;      end    % Number of iteration for stagnation
...
while (i <= maxit)&&(stagcnt <= staglim)
    ...
    %t_alph_est=lscov(Lmat,tijMat(:),diag(wMat));
    t_alph_est=(Lmat'*wMat*Lmat)\(Lmat'*wMat*tijMat(:));
    %% Update IRLS weights
    residual_vec = reshape(Lmat*t_alph_est-tijMat(:),d,ss_num);
    ...
    wMat = kron(sparse([1:ss_num]',[1:ss_num]',1./(max(residual_norms.^(2-opts.p), delt)),ss_num,ss_num,ss_num),speye(d));
    
    %% Check convergence of cost function
    cost_val = sum(residual_norms.^opts.p);
